...
These markdown files in `.cursor/agents/` are **Cursor subagents**. Agent can delegate to them for specialized work. Each runs in its own context window.

## How to use

- **Invoke by name**: In chat, use `/name` or ask naturally, e.g. `/engineering-frontend-developer` or “Have the design-ui-designer subagent suggest a layout.”
- **Automatic delegation**: Agent uses the `description` in each file’s frontmatter to decide when to delegate. Descriptions are in the table below.
...
- **YAML frontmatter**: `name`, `description`, `model` (optional: `readonly`, `background`)
